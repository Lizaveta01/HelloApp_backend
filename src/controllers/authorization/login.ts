...
const { PASSWORD_WRONG, USER_NOT_FOUND, USER_BLOCKED, AUTH_ERROR } = ServerResponse;
...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        if (!validationPassword) {
            return res.status(400).send(createError(401, PASSWORD_WRONG));
        }

        if (user.status === 'blocked') {
            return res.status(403).send(createError(403, USER_BLOCKED));
        }
